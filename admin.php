...
if (file_exists($CheminFichier)) {
    if (!unlink($CheminFichier)) {
        echo "Problème lors de l'effacement de $CheminFichier<br />\n";
    } else {
        echo "Fichier $CheminFichier effacé.<br />\n";
    }
} else {
...
<table class="table table-dark table-bordered table-hover" style="text-align:center">
    ...
    <?php
        ...
        while ($data = $requete->fetch()) {
            if ($data['statutChoixChanson'] === 1 && $data['statutEnvoiChanson'] === 1 && $data['statutPaiement'] === 1) {
            ...
            if ($data['statutChoixChanson'] === 1) {
                echo "<td class='text-success h5'>" . $data['statutChoixChanson'] . "</td>";
            } else {
                echo "<td>" . $data['statutChoixChanson'] . "</td>";
            }
            if ($data['choixSupprime'] === 1) {
                echo "<td class='text-danger h5'>" . $data['choixSupprime'] . "</td>";
            } else {
                echo "<td>" . $data['choixSupprime'] . "</td>";
            }
            if ($data['statutEnvoiChanson'] === 1) {
                echo "<td class='text-success h5'>" . $data['statutEnvoiChanson'] . "</td>";
            } else {
                echo "<td>" . $data['statutEnvoiChanson'] . "</td>";
            }
            if ($data['envoiSupprime'] === 1) {
                echo "<td class='text-danger h5'>" . $data['envoiSupprime'] . "</td>";
            } else {
                echo "<td>" . $data['envoiSupprime'] . "</td>";
            }
            if ($data['statutPaiement'] === 1) {
                echo "<td class='text-success h5'>" . $data['statutPaiement'] . "</td>";
            } else {
...
$requete = $connexion->query("SELECT user.id,email,statutChoixChanson,statutEnvoiChanson,statutPaiement,artiste,titre,img,extrait,chansonEnvoyée FROM `user` INNER JOIN `choixMusique` ON user.id = choixMusique.id_user");
    ...
    while ($data = $requete->fetch()) {
            ...
            '<td style="padding-top:20px"> 
                ...
                <div class="modal fade" id="exampleModal' . $data['id'] . '" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-body">
                            ...
                            <audio controls="controls" class=" w-100 text-center"> <source src=' . $data["extrait"] . '></audio>
                        </div>';
        ...
        // Si le choix est validé on affiche le message 'Choix de la chanson validé' avec une possibilité d'annuler en cas d'erreur
        ...
        if ($data['statutEnvoiChanson'] === 0 && $data['statutChoixChanson'] === 1) {
            ...
            if ($data['chansonEnvoyée'] !== 'NULL') {
                echo "<a  class='btn btn-danger btn-sm mx-2' href='admin.php?supprimeEnvoi=" . $data['id'] . "'>Supprimer</a> </td>";
            }
        ...
        if ($data['statutEnvoiChanson'] === 1 && $data['statutChoixChanson'] === 1  && $data['statutPaiement'] === 0) {
            ...
            echo "<td style='padding-top:20px' class='text-success'>Paiement effectué et validé<br> <a class='btn-outline-warning btn-sm' href='admin.php?annulePaiement=" . $data['id'] . "' >Annuler?</a></td>";
        }
